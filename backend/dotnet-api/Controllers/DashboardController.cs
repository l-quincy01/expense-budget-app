...
public class DashboardController : ControllerBase
{
    ...
    // --------------------------------------------------------
    // CREATE DASHBOARD  (REST: POST /api/dashboard)
    // --------------------------------------------------------
    [HttpPost]
    [RequestSizeLimit(200 * 1024 * 1024)]
    public async Task<IActionResult> CreateDashboard(
        ...
        CancellationToken ct)
    {
        ...
        var client = _httpClientFactory.CreateClient("AiIngest");
        var nodeApiUrl = _config["NodeIngest:BaseUrl"]
                         ?? "http://localhost:4010/api/dashboards";
    ...
    // --------------------------------------------------------
    // UPDATE DASHBOARD  (REST: PUT /api/dashboard/{dashboardName})
    // --------------------------------------------------------
    ...
    // --------------------------------------------------------
    // DELETE DASHBOARD (REST: DELETE /api/dashboard/{dashboardName})
    // --------------------------------------------------------
    [HttpDelete("{dashboardName}")]
    public async Task<IActionResult> DeleteDashboardByName([FromRoute] string dashboardName, CancellationToken ct)
    {
