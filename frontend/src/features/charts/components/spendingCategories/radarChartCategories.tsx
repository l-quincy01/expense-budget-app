"use client";

import { PolarAngleAxis, PolarGrid, Radar, RadarChart } from "recharts";

import {
  ChartConfig,
  ChartContainer,
  ChartTooltip,
  ChartTooltipContent,
} from "@/components/ui/chart";
import { radarMonthsChartData } from "@/utils/sumMonths";
import { radarCategoriesChartData } from "@/utils/chart/barchart/categories/sumCategories";
import { ChartCard } from "@/features/charts/components/chart-card";

export const description = "A radar chart with dots";

interface props {
  spendingCategories: radarCategoriesChartData[];
  spendingMonths: radarMonthsChartData[];
}

export function RadarChartCategories({
  spendingCategories = [],
  spendingMonths = [],
}: props) {
  const chartConfig = {
    category: {
      label: "Category",
      color: "var(--chart-1)",
    },
  } satisfies ChartConfig;

  return (
    <ChartCard
      title="Spending overview"
      description="Overview of Spending"
      className="border-[0.5px] px-1"
      headerClassName="flex flex-col justify-center px-4"
      contentClassName="pb-0 px-0 py-0 w-full grid grid-cols-2"
    >
        <ChartContainer config={chartConfig} className=" px-0 py-0">
          <RadarChart data={spendingCategories}>
            <ChartTooltip cursor={false} content={<ChartTooltipContent />} />
            <PolarAngleAxis dataKey="category" />
            <PolarGrid />
            <Radar
              dataKey="total"
              fill="var(--color-category)"
              fillOpacity={0.6}
              dot={{
                r: 4,
                fillOpacity: 1,
              }}
            />
          </RadarChart>
        </ChartContainer>
        {spendingMonths.length == 0 ? (
          <></>
        ) : (
          <ChartContainer config={chartConfig} className=" px-0 py-0">
            <RadarChart data={spendingMonths}>
              <ChartTooltip cursor={false} content={<ChartTooltipContent />} />
              <PolarAngleAxis dataKey="month" />
              <PolarGrid />
              <Radar
                dataKey="total"
                fill="var(--color-category)"
                fillOpacity={0.6}
                dot={{
                  r: 4,
                  fillOpacity: 1,
                }}
              />
            </RadarChart>
          </ChartContainer>
        )}
    </ChartCard>
  );
}
